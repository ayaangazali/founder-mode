...
(async () => {
  ...
  // ---------------- 390x664 portrait ----------------
  {
    ...
    await page.waitForTimeout(200);
    check('390x664: GOT IT closes the card and NOTHING persists (chip returns every title visit — owner rule v3)', await page.evaluate(() => {
    ...
    // ✕ hides for this title visit only; a reload brings the chip back
    await page.reload(); await page.waitForTimeout(900);
    check('390x664: chip returns on the next visit after GOT IT', await page.evaluate(() => document.getElementById('a2hs').style.display) === 'block');
    check('390x664: ✕ hides for the visit without persisting', await page.evaluate(() => {
